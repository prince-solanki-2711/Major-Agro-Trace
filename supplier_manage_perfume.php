...
if(isset($_POST["btnsave"])) {
    ...
    $query = "INSERT INTO product_detail VALUES($pid,'$name','$catid','$desc','$price','$ipath','$sid')";
    if(mysqli_query($con, $query)) {
        move_uploaded_file($tpath, $ipath);
        echo "<script>alert('Harvest Item Saved Successfully'); window.location='supplier_manage_perfume.php';</script>";
    }
}
...
if(isset($_REQUEST['dpid'])) {
    ...
    if(mysqli_query($con, $query)) {
        echo "<script>alert('Produce Item Removed'); window.location='supplier_manage_perfume.php';</script>";
    }
}

// --- 3. PREPARE EDIT MODE ---
$name1 = $cid1 = $desc1 = $price1 = $pimg1 = "";
if(isset($_REQUEST['epid'])) {
    $pid1 = $_REQUEST['epid'];
    $res5 = mysqli_query($con, "SELECT * FROM product_detail WHERE product_id = '$pid1'");
    $r5 = mysqli_fetch_array($res5);
    $name1 = $r5[1]; $cid1 = $r5[2]; $desc1 = $r5[3]; $price1 = $r5[4]; $pimg1 = $r5[5];
}
...
if(isset($_POST["btnupdate"])) {
    ...
    if($_FILES["txtimg"]["size"] > 0) {
        ...
        $ipath = "perfume_img/" . time() . ".png";
        move_uploaded_file($tpath, $ipath);
        $query = "UPDATE product_detail SET product_name='$name', category_id='$cid', product_description='$desc', product_price='$price', product_image='$ipath' WHERE product_id='$pid'";
    } else {
        $query = "UPDATE product_detail SET product_name='$name', category_id='$cid', product_description='$desc', product_price='$price' WHERE product_id='$pid'";
    }
    if(mysqli_query($con, $query)) {
        echo "<script>alert('Harvest Details Updated'); window.location='supplier_manage_perfume.php';</script>";
    }
}
?>
        ...
        <div class="col-lg-4 mb-4">
            <div class="form-card animate__animated animate__fadeInLeft">
                ...
                <form method="post" name="form1" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Product Name (e.g. Organic Alphanso)</label>
                    ...
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Farming Description / Certifications</label>
                        <textarea class="form-control" name="txtdesc" rows="3" required style="border-radius: 10px;"><?php echo $desc1; ?></textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Market Price (₹)</label>
                        <input type="number" class="form-control" name="txtprice" value="<?php echo $price1; ?>" required style="border-radius: 10px;">
                    </div>

                    <div class="mb-4">
                        <label class="form-label small fw-bold text-muted">Product Visual (Harvest Image)</label>
                    ...
                    <?php if(isset($_REQUEST['epid'])): ?>
                        ...
                        <a href="supplier_manage_perfume.php" class="btn btn-link w-100 mt-2 text-decoration-none text-muted small">Discard Changes</a>
                    <?php else: ?>
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <img src="<?php echo $r3[5]; ?>" class="prod-img-preview me-3" onerror="this.src='https://cdn-icons-png.flaticon.com/512/1147/1147805.png';">
                                        <div>
                                            <div class="fw-bold text-dark"><?php echo $r3[1]; ?></div>
                                            <small class="text-muted d-block" style="font-size: 0.75rem;">Batch ID: #AG-<?php echo $r3[0]; ?></small>
                                        </div>
                                    </div>
                                </td>
                                <td><span class="badge-harvest"><?php echo $cat_row[0]; ?></span></td>
                                <td><span class="price-badge">₹<?php echo number_format($r3[4], 2); ?></span></td>
                                <td class="text-end">
                                    <a href="supplier_manage_perfume.php?epid=<?php echo $r3[0]; ?>" class="btn btn-sm btn-outline-success me-1" title="Edit Harvest">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <a href="supplier_manage_perfume.php?dpid=<?php echo $r3[0]; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Remove this organic item from marketplace?')" title="Delete Item">
